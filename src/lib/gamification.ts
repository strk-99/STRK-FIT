import { eachWeekOfInterval, endOfWeek, parseISO, differenceInDays, startOfDay } from 'date-fns';
import type { DailyLog, DayShiftState } from '../store';

export type UserLevel = 'Survivor' | 'Architect' | 'In Control';

export const LEVEL_MAP: Record<number, UserLevel> = {
    1: 'Survivor',
    2: 'Architect',
    3: 'In Control'
};
...
export const LEVEL_DESCRIPTIONS: Record<number, string> = {
    1: 'Rebuilding consistency after years of struggle. This phase is about showing up — not pushing limits.',
    2: 'Building a routine that fits your life. Establishing patterns that work.',
    3: 'Food, movement, and sleep aligned. Living in sustainable balance.'
};

export const UNLOCK_CRITERIA: Record<number, string> = {
    1: 'You are here',
    2: 'Unlocks after 4 consistent weeks (not required to be consecutive)',
    3: 'Unlocks after 12 consistent weeks (not required to be consecutive)'
};

export function calculateUserLevel(logs: Record<string, DailyLog>): { level: number; title: UserLevel; progress: number } {
    // If no logs, level 1
    const dates = Object.keys(logs).sort();
    if (dates.length === 0) return { level: 1, title: 'Survivor', progress: 0 };

    // Calculate "Weeks Active"
    // Active Week = >= 3 logs in that week (showing up consistently)

    // Find earliest and latest log
    const firstLog = parseISO(dates[0]);
    const today = new Date();

    const weeks = eachWeekOfInterval({ start: firstLog, end: today }, { weekStartsOn: 1 });

    let activeWeeks = 0;

    weeks.forEach(weekStart => {
        const weekEnd = endOfWeek(weekStart, { weekStartsOn: 1 });
        const logsInWeek = dates.filter(d => {
            const date = parseISO(d);
            return date >= weekStart && date <= weekEnd;
        });

        // Count as active week if >= 3 logs
        if (logsInWeek.length >= 3) {
            activeWeeks++;
        }
    });

    // New compassionate level system (3 levels only)
    let level = 1;
    if (activeWeeks >= 4) level = 2;  // After 4 consistent weeks
    if (activeWeeks >= 12) level = 3; // After 12 consistent weeks (~3 months)

    return {
        level,
        title: LEVEL_MAP[level] || 'Survivor',
